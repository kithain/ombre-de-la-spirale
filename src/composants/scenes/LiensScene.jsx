...
/**
 * Obtient les classes CSS de couleur pour un PNJ selon sa origine
 */
const obtenirClassesCouleurPnj = (origine, pnj) => {
  // Si c'est un monstre du bestiaire avec estEnnemi: true, utiliser le rouge
  if (origine === "bestiary" && pnj && pnj.categorie === "ennemis") {
...
function LiensScene({ lieuLie, pnjLies = [] }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2 text-xs">
      ...
      {pnjLies.map((pnj, index) =>
        pnj.id && !pnj.introuvable ? (
          <button
            key={`${pnj.id}-${index}`}
            type="button"
            titre={pnj.id}
          ...
          <span
            key={`${pnj.id || pnj.nom}-${index}`}
            className="inline-flex items-center gap-1 px-2 py-1 border border-orange-700/50 text-orange-400/80 bg-orange-950/20"
            titre="PNJ introuvable"
          >
            <User className="w-4 h-4" />
            {pnj.nom} ⚠
          </span>
