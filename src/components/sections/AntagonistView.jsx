    ...
    <div className="space-y-6 animate-fadeIn">
          ...
          <button
            ...
            className={cn(
              ...
              showEnemiesOnly
                ...
                : "border-stone-700 text-stone-300 hover:border-amber-700 hover:text-amber-200"
            )}
          >
      ...
      {noScenes.length > 0 && (
        <Card className="border-amber-900/40 bg-amber-950/10">
          <h4 className="text-sm font-mono tracking-widest text-amber-300 uppercase border-b border-amber-900/40 pb-2 mb-3">
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            {noScenes.map((npc) => (
              <span
                key={npc.id}
                className="px-2 py-1 rounded border border-amber-900/50 text-amber-200 bg-amber-900/10"
      ...
      {spiralMineBestiary.length > 0 && (
        <Card className="border-amber-900/40 bg-amber-950/10">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-sm font-mono tracking-widest text-amber-300 uppercase">
            ...
            <span className="text-[11px] text-amber-200 border border-amber-900/50 px-2 py-0.5 rounded">
              {spiralMineBestiary.length} entrées
            </span>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm text-amber-100">
              <thead className="text-xs uppercase tracking-widest text-amber-300">
              ...
              <tbody className="divide-y divide-amber-900/30">
                {spiralMineBestiary.map((b) => (
                  <tr key={b.id} className="align-top">
                    ...
                    <td className="py-2 pr-2 text-amber-200">FP {b.fp ?? "–"}</td>
                    ...
                    <td className="py-2 text-amber-300 text-xs">{b.env || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {filtered.map((npc) => (
          ...
          >
            <button
              ...
              className="w-full text-left flex items-start justify-between gap-3 border-b border-stone-800 pb-3 mb-3 hover:border-amber-800 transition-colors"
            >
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-serif text-amber-200">{npc.name}</h3>
                  {npc.isEnemy && (
                ...
                <p className="text-sm text-stone-400">{npc.role}</p>
                <p className="text-xs text-stone-500 mt-1">
                  {npc.zone?.name} · {npc.loc?.name}
                </p>
              </div>
              <span className="text-[11px] text-amber-300 font-mono px-2 py-1 rounded border border-amber-900 bg-amber-950/30">
                {openId === npc.id ? "–" : "+"}
              </span>
            </button>
            ...
            {openId === npc.id && (
              <div className="space-y-3">
                {bestiaryMap.has(npc.id) && (
                  <div className="border border-amber-900/40 rounded bg-amber-950/10 p-3 space-y-2">
                    <button
                      type="button"
                      onClick={() => toggleBestiary(npc.id)}
                      className="w-full text-left flex items-center justify-between gap-2 text-sm text-amber-200"
                    >
                      <span className="font-semibold uppercase tracking-widest text-[12px]">
                        Bestiaire
                      </span>
                      <span className="text-[11px] px-2 py-0.5 rounded border border-amber-800 bg-amber-900/20">
                      ...
                      (() => {
                        ...
                        return (
                          <div className="space-y-1 text-sm text-amber-100">
                            <div className="flex flex-wrap gap-2 text-[12px] text-amber-300 uppercase tracking-widest">
                            ...
                            <div className="flex flex-wrap gap-3 text-[12px] text-amber-200">
                              {pv !== undefined && (
                            ...
                            {Array.isArray(qualites) && qualites.length > 0 && (
                              <p
                                className="text-[12px] text-amber-200 underline decoration-dotted"
                                title={tooltip.qualites}
                              >
                                SQ : {qualites.slice(0, 2).join(" · ")}
                              </p>
                            )}
                            <div className="grid grid-cols-2 gap-2 text-[12px] text-amber-200 pt-1">
                              {env && (
                            ...
                            {Array.isArray(attSpe) && attSpe.length > 0 && (
                              <p title={tooltip.attSpe} className="text-[12px] text-amber-200">
                                Att. spé. : {attSpe.join(" · ")}
                              </p>
                            )}
                            {competences && (
                              <p title={tooltip.competences} className="text-[12px] text-amber-200">
                                Compétences : {competences}
                              </p>
                            )}
                            {dons && (
                              <p title={tooltip.dons} className="text-[12px] text-amber-200">
                                Dons : {dons}
                              </p>
                            )}
                            {tactiques && (
                              <p title={tooltip.tactiques} className="text-[12px] text-amber-200">
                                Tactiques : {tactiques}
                              </p>
                            )}
                            {butin && (
                              <p title={tooltip.butin} className="text-[12px] text-amber-200">
                                Butin : {butin}
                              </p>
                            )}
                          </div>
                ...
                {npc.details?.length > 0 && (
                  <div className="space-y-1">
                    <h4 className="text-xs uppercase tracking-widest text-stone-500">Caractéristiques</h4>
                    <ul className="space-y-1 text-sm text-stone-300">
                      {npc.details.map((d, idx) => (
                        <li key={idx} className="flex gap-2">
                          <span className="mt-1 w-1 h-1 rounded-full bg-stone-600 block flex-shrink-0" />
                          <span>{d}</span>
                        </li>
                      ))}
                    </ul>
                ...
                {npc.occurrences.length > 0 ? (
                  <div className="space-y-1">
                    <h4 className="text-xs uppercase tracking-widest text-stone-500">
                      Apparitions
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {npc.occurrences.map((occ, idx) => (
                        <Link
                          ...
                          className="px-2 py-0.5 rounded border border-stone-700 bg-stone-900/60 text-[11px] hover:border-amber-700 hover:text-amber-200 transition-colors"
