...
logs_to_epub.py
---------------
Convertit tous les fichiers .md du dossier cible en un seul EPUB lisible sur tablette.

Usage :
    python logs_to_epub.py

Dépendances :
    pip install ebooklib markdown

Configuration :
    Modifier les variables LOG_DIR et OUTPUT_FILE ci-dessous.
"""
...
# ─── CONFIGURATION ────────────────────────────────────────────────────────────
LOG_DIR     = Path(r"D:\script\Scénario\V4\docs")   # Dossier contenant vos .md
...
def extract_log_number(filename: str) -> int:
    """Extrait le numéro de log depuis le nom de fichier (ex: log_01_... → 1)."""
    m = re.search(r"log[_\-]?(\d+)", filename, re.IGNORECASE)
...
def build_epub(log_dir: Path, output_file: Path) -> None:
    ...
    md_files = sorted(
        [f for f in log_dir.glob("*.md") if re.search(r"log[_\-]?\d+", f.name, re.IGNORECASE)],
        key=lambda f: extract_log_number(f.name),
    )

    if not md_files:
        sys.exit(f"❌  Aucun fichier log_*.md trouvé dans : {log_dir}")
