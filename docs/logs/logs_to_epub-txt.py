...
OUTPUT_EPUB = LOG_DIR / "campagne_complete.epub"     # Fichier EPUB de sortie
OUTPUT_TXT  = LOG_DIR / "campagne_complete.txt"      # Fichier TXT de sortie
...
def collect_md_files(log_dir: Path) -> list[Path]:
    """Liste et trie les fichiers .md numérotés (ex: 01-nom.md)."""
    md_files = sorted(
...
def build_txt(md_files: list[Path], output_file: Path) -> None:
    print(f"📝  Construction du TXT ({len(md_files)} fichier(s))...")
    ...
    for md_file in md_files:
        ...
        # Retire les symboles Markdown les plus courants pour un texte brut lisible
        body = raw
        body = re.sub(r"^#{1,6}\s+.*$", "", body, count=1, flags=re.MULTILINE)  # retire le H1 déjà utilisé comme titre
        body = re.sub(r"^#{1,6}\s+", "", body, flags=re.MULTILINE)             # autres titres -> texte simple
        body = re.sub(r"\*\*(.+?)\*\*", r"\1", body)                            # gras
        body = re.sub(r"\*(.+?)\*", r"\1", body)                                # italique
        body = re.sub(r"^>\s?", "", body, flags=re.MULTILINE)                   # citations
        body = re.sub(r"^[-*]\s+", "• ", body, flags=re.MULTILINE)              # listes
        body = body.strip()

        parts.append(f"\n\n{'=' * 60}\n{chapter_title.upper()}\n{'=' * 60}\n\n{body}")

    output_file.write_text("\n".join(parts), encoding="utf-8")
    print(f"\n✅  TXT créé : {output_file}")
    print(f"   Taille : {output_file.stat().st_size // 1024} Ko")


def build_epub(md_files: list[Path], output_file: Path) -> None:
    print(f"📖  Construction de l'EPUB ({len(md_files)} fichier(s))...")
...
if __name__ == "__main__":
    ...
    files = collect_md_files(LOG_DIR)
    print(f"📂  {len(files)} fichier(s) trouvé(s) dans {LOG_DIR}\n")

    build_epub(files, OUTPUT_EPUB)
    print()
    build_txt(files, OUTPUT_TXT)